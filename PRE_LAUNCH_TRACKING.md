...
- Chrome management module — detect / launch / close / activity / paths, per-OS dispatch — `bef9937`
- Notifications module (Win toast / macOS osascript / Linux notify-send), with `[notify:*]` logging — `bef9937`
- Async pairing API: `request_pairing` returns immediately, `check_pairing_status` MCP tool, persisted pending pairings — Wave 1 / `2858fe3`-era
- Multi-extension WebSocket support + per-profile routing via `extension-bridge.js` map — `11b5074`
- Extension hello handshake (`installId`, `gaiaEmail`, `profileId`) drives server-side profile registration — `13a0002`, `fccde12`, `f439589`
- `browser_create_tab` is the readiness gate; calls `ChromeManager.ensureReady` — `6dff5e7`
- Per-agent MCP tool routing to the agent's bound profile — `64e7290`
- `POST /api/ui/profiles` profile-name validation + sanitization — `dd38647`
- `POST /api/ui/agents` direct-create endpoint + `PATCH /api/ui/agents/:key` profile-binding update — web UI / pair-modal commits (`efc5b94`, `2fbaa9a`)
- `profileId` wired through approve flow; supports `__new__` sandbox profile creation — `9df5bef`
- Removed legacy `set_network_mode` extension WS handler (replaced by web-UI restart) — `e6502de`
- Approve/deny return `null` on terminal states; routes 409/404 — `f2572fb`
- `clearConnection(ws)` only clears the matching profile, not all — `5d5e14d`
- Web UI is localhost-only, no API key required — `0cf67de`
- Web UI served via manual `fs.readFileSync` for pkg snapshot compat — `e5c3d8f`
- Pending pairings expire after 24 h — `7c1fb59`
- Removed dead `browser_create_tab` switch case — `d93e78e`
- Auto-open Web UI in default browser on `--foreground` start — `aeafddc`
- Removed orphan `getLocalIP()` (post-C5 cleanup) — `ece5dbd`
- Per-profile `webPilotStatus` (active / ready / needs_setup) on `/api/ui/status` — `8930d46`
- `validateKey()` in-memory cache, mtime-invalidated; no more double disk-read per tool call — `22d4630`
- MCP `serverInfo.name='WebPilot'` in initialize handshake — `a1af1c3`
- `request_pairing` short-circuits when caller is already keyed — `674003b`, `a6833e5`
- Dropped dead `profileDisplayName` storage key; surface `--network` in CLI help — `c441118`

### Web UI

- Phased rebuild: design foundation, per-page IA, server-backed pairing/notifications/Chrome controls, microcopy + skeletons (Phases 1-4) — `769d2a4`, `aa9089d`, `0f5543c`, `cc35ada`
- Icon system swap Phosphor → Heroicons — `26b9789`
- Structural redesign + three togglable palettes; mono palette locked in as default — `df8b38a`, `49d1254`, `3f38e3d`, `8b48c94`
- Pair-agent slim bar + modal; modal pre-provisions API keys; profile bindings everywhere; contextual back nav — `fb89c52`, `efc5b94`, `2fbaa9a`
- Themed confirm modal replaces `window.confirm()` — `cd2041a`
- Per-agent `.mcp.json` copy snippet + setup instructions — `f1d96a5`
- Stale-REST-vs-WS race guard applied to Home + Profiles pages — `c04bdc5`, `f87e1b1`
- Dashboard Action Items replace KPI grid; inline approve/deny — `3c06662`
- First-time setup card; pairing instructions in MCP initialize — `eaf5726`
- Ready badge uses the blue accent — `6f594c5`
- Per-profile Active/Ready/Needs-Setup state surfaced — `67e299a`

### Chrome extension

- Removed Pairing tab + paired-agents view from popup — `dbe6f2f`
- Removed network-mode toggle from popup — `b288ef7`
- Multi-extension WS handshake + profile self-identify via `hello` (`installId`, `gaiaEmail`) — `11b5074`, `13a0002`
- Extension allows user to change picked profile (C2/C3 from extension review) — `57d7f1a`
- Hello handshake must precede other messages on WS open — `904b696`
- Removed zombie `set_pairing_required` cache (web UI owns this state) — `595df13`
- Popup recovers from manual disconnect + Chrome restart — `bdae1ac`
- `windows-detector` PowerShell script no longer loses output to inline `#` comment — `7171453`

### Notifications

- AppUserModelID self-registration before Windows toast — `d2c8801`
- Windows toast is clickable, opens WebPilot UI — `754ec8e`

### Documentation

- Full doc audit: technical docs synced to branch behaviour; claim-by-claim corrections; verification-audit follow-ups — `830c294`, `595be70`, `281598d`

### Tooling / chore

- Removed `beads` issue-tracker integration; tracker is now GitHub Issues — `2858fe3`
- Unified all package versions to 1.0.0 — `11b5074` era / `chore: unify all package versions`
- pkg build pipeline bundles `server-web-ui/out` via assets — `c9f7ed0`
