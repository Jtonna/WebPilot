...
- ~~**No MCP tool for inspecting formatter errors during iteration.**~~ ✅ `webpilot_dev_get_formatter_logs({ platform, limit?, errors_only? })` ships in this branch. Returns the health summary + recent ring-buffer entries (incl. stack traces, workflow name, params, tabId for workflow errors). No auth required — read-only, consistent with `webpilot_get_formatter_info`. Documented in the MCP server's `instructions` block and in `accessibility-tree-formatters/DEV_GUIDE.md`.
