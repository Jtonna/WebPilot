...
- ~~**Single shared `apiKey` reused for extension transport + UI admin.**~~ ✅ Mitigated in `8905b0d` (UI-admin half), then **fully retired 2026-05-17** as a hard cutover (Phases A + B + D from `docs/SECURITY_AUDIT_2026-05-17.md`). The shared transport key is gone: `getApiKey()` / `DEFAULT_API_KEY` / `API_KEY = getApiKey()` deleted; `createServer({ apiKey })` and `createMcpHandler(..., apiKey, ...)` lost the parameter; `GET /connect` no longer returns an `apiKey` field; extension WS handshake is `?installId=<uuid>` instead of `?apiKey=...`; popup endpoints auth via `X-Install-Id`; the `_authPopup` transport-key fallback from `7dc391d` is gone. Extension bumped to `1.2.0`; one-time chrome://extensions/ reload per profile required. Phase C (auto-provision per-profile paired-agent key) was dropped — popup operates in profile-context, not agent-context, so synthetic agents aren't needed. MCP `tools/call` paired-agent auth is unchanged.
...
- ~~**Phase 6 — Extension popup redesign.**~~ ✅ `4009982`. Popup gutted to 4 components (connection dot / current tab / Block-Allow toggle / Open dashboard). Theme matched to webapp. Extension bumped to `1.1.4` (then to `1.2.0` in the 2026-05-17 auth cutover, `f7f2bb8`).
