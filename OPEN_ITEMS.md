...
- **Extension V2: thin scaffolding + server-side handler logic.** Today every behavioral fix to `click.js` / `keyboard.js` / `accessibility.js` etc. requires a manual `chrome://extensions/` reload per Chrome profile — friction compounds with the number of paired profiles. Move all click / type / scroll / accessibility-tree logic to the server (where formatters and workflows already hot-reload), shrink the extension to a thin CDP relay exposing `cdp_send` + `cdp_send_batch` + tab lifecycle. After a one-time unpacked-reload per profile to install V2, every future improvement to handler behavior lands by restarting the server (or via a new `webpilot_dev_reload_handlers`) — no chrome://extensions/ click ever again under normal iteration. Full design + phased migration plan + agent-swarm wedge layout in [`docs/EXTENSION_V2_DESIGN.md`](docs/EXTENSION_V2_DESIGN.md). Phases: A (foundation: protocol + shared detach/stall util) → B (per-handler migration behind `useV2Handlers` config flag, type → scroll → click → a11y) → C (cutover: drop legacy cases from extension, bump to v1.3.0). Marks V2 of the extension.
