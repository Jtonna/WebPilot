...
- **`browser_click` returns `MCP error -32000: Command timeout` while the click actually succeeded.** Root cause identified: `handlers/click.js` performs many `chrome.debugger.sendCommand` roundtrips (path iteration, visual cursor, ripple, cleanup). When the click triggers SPA navigation, the CDP target frame is replaced mid-flight; subsequent `Runtime.evaluate` calls hit a context being torn down — the promise **neither resolves nor rejects**, the `await` hangs forever, the handler never reaches `return`, no response is sent, server times out at 30s (`extension-bridge.js:19 COMMAND_TIMEOUT`). The `.catch(() => {})` on cleanup only catches rejections, not perpetual pending. Fix candidates: (1) listen for `chrome.debugger.onDetach` and resolve early on detach — surgical, addresses root cause; (2) decouple visual-cursor cleanup from the click result, return success immediately after `mouseReleased` — cleaner UX; (3) per-CDP-call timeout — risks masking other bugs. Lean: (1) + (2) together. (Found and diagnosed during live test 2026-05-16.)
- **Discord formatter abstracts composer accessible name** (`Message @notboosted` in raw a11y tree → `Message textbox` in formatted output). The formatter's `notes` field still claims the composer name starts with `Message #` or `Message @` — but the formatted tree no longer carries that string. The `send_message` workflow happens to use `name: 'Message textbox'` as its primary selector so it still works, but: (a) update the notes wording to match the formatted output, or (b) preserve the discriminating part of the name in the formatted output (helpful for agents disambiguating multiple textboxes). (Found during live test 2026-05-16.)
