...
CreateDirectory "$LOCALAPPDATA\VantageFeed\extension"
CopyFiles /SILENT "$INSTDIR\resources\extension\*.*" "$LOCALAPPDATA\VantageFeed\extension"
```

**Add to `customUnInstall`:**
```nsis
; Remove extension folder
RMDir /r "$LOCALAPPDATA\VantageFeed\extension"
RMDir "$LOCALAPPDATA\VantageFeed"  ; Remove parent if empty
```
...
```json
{
  ...
  "loaded": true,
  "loadedVersion": "0.2.0",
  "extensionPath": "C:\\Users\\...\\VantageFeed\\extension"
}
```

**Detection logic:**
1. **deployed** - Check `%LOCALAPPDATA%\VantageFeed\extension\manifest.json` exists
  ...
  "extensions": {
    "settings": {
      "cohbniildehaagapenmhebelodcmpjbp": {
        "location": 4,
        "path": "C:\\Users\\J\\...\\vantage-feed-extension\\unpacked-extension",
...
- To find our extension: search `extensions.settings` for entry where `path` contains `VantageFeed`

## Notes

- Extension folder path: `%LOCALAPPDATA%\VantageFeed\extension\` (Windows)
